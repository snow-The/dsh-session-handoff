...
import { readSessionEventsFile as readFastEvents } from './zstd-reader.js';
...
export function autoIngestSession(sessionId, cwd) {
  try {
    ...
    if (!db) return null;
    const fpath = sessionFilePath(sessionId, cwd);
    const events = fpath ? readSessionEventsFromFile(fpath) : [];
    const wm = readWatermarks();
    const added = ingestSession(db, sessionId, events, wm, false);
    writeWatermarks(wm);
    const s = graphStats(db);
    return { added, nodes: s.nodes, edges: s.edges, checkpoints: s.checkpoints };
  } catch (e) {
