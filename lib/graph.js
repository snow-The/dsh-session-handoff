...
export function readSessionEventsFromFile(path) {
  ...
  try {
    if (path.endsWith('.zstd')) {
      // 1) node:zlib native Zstd (Node 22.13+) — zero deps, always available
      //    when DSH runs on a modern Node; no CLI, no PATH concerns.
      const { zstdDecompressSync } = require('node:zlib');
      if (typeof zstdDecompressSync === 'function') {
        text = zstdDecompressSync(readFileSync(path)).toString('utf8');
      } else {
        // 2) fallback: zstd CLI — PATH first, then known winget install path
