...
import { readSessionEventsFile as readFastEvents, readSessionEventsIncremental, fileStamp, readHeaderLine } from './zstd-reader.js';
...
export function sessionFilePath(sessionId, cwd) {
  // Runtime session.id carries a 'session-' prefix (diag: session=session-644ee781-…)
  // while on-disk dirs use the bare uuid. Normalize before joining.
  sessionId = String(sessionId ?? '').replace(/^session-/, '');
  ...
  if (cwd) {
    ...
    // DSH migrates a session to a new generation file (session.v<N>.jsonl.zstd)
    // and keeps appending THERE - the highest generation is the live log. Reading
    // only session.jsonl.zstd would freeze this plugin at the migration moment.
    try {
      ...
      for (const entry of readdirSync(dir)) {
        ...
        if (m && Number(m[1]) > best) { best = Number(m[1]); bestName = entry; }
      }
      if (bestName) candidates.push(join(dir, bestName));
    } catch { /* dir absent: fall through to the fixed names */ }
...
export function readSessionEventsFromFile(path) {
  if (!path) return [];
  // fast path: multi-frame in-process zstd reader (node:zlib per frame)
  const fast = readFastEvents(path);
...
const SQL = {
  ...
  // --- provenance: who contributed what (multi-context / multi-agent axis) ---
  // All additive: existing tables and queries are untouched, and the network view
  // that notemap builds can be dropped and rebuilt from these at any time.
  sources: 'CREATE TABLE IF NOT EXISTS sources (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT UNIQUE NOT NULL, parent_session TEXT, agent_kind TEXT NOT NULL DEFAULT \'main\', cwd TEXT, label TEXT, created_at INTEGER, last_seen INTEGER)',
  mentions: 'CREATE TABLE IF NOT EXISTS mentions (source_id INTEGER NOT NULL, node_id TEXT NOT NULL, checkpoint_seq INTEGER NOT NULL, count INTEGER NOT NULL DEFAULT 1, first_seen INTEGER, last_seen INTEGER, PRIMARY KEY (source_id, node_id, checkpoint_seq))',
  aliases: 'CREATE TABLE IF NOT EXISTS aliases (alias TEXT PRIMARY KEY, node_id TEXT NOT NULL, kind TEXT, created_at INTEGER)',
  delegations: 'CREATE TABLE IF NOT EXISTS delegations (parent_source INTEGER NOT NULL, child_source INTEGER NOT NULL, PRIMARY KEY (parent_source, child_source))',
  mentions_node: 'CREATE INDEX IF NOT EXISTS idx_mentions_node ON mentions(node_id)',
  mentions_source: 'CREATE INDEX IF NOT EXISTS idx_mentions_source ON mentions(source_id)',
};
...
export function openDb() {
  ...
  // Concurrent access: the ingest worker and the main thread share this file.
  db.exec('PRAGMA busy_timeout = 5000');
  for (const s of Object.values(SQL)) db.exec(s);
  try { ensureProvenance(db); } catch (e) { console.warn('[dsh-session-handoff] provenance backfill failed:', String(e?.message ?? e)); }
  return db;
}

/** Highest-generation log inside one session directory, or null. */
function pickGenerationFile(dir) {
  let best = null;
  let bestVersion = -1;
  try {
    for (const entry of readdirSync(dir)) {
      ...
      if (version > bestVersion) { bestVersion = version; best = join(dir, entry); }
    }
  } catch { return null; }
  return best;
}
...
/**
 * Materialise provenance ONCE - the multi-context / multi-agent axis.
 *
 * sources    : one row per session/agent log under <DSH_HOME>/sessions (agent_kind
 *              from the header's origin/delegationDepth, parent from
 *              header.parentSession - the subagent tree was always on disk, it had
 *              simply never been turned into graph data).
 * mentions   : which source mentioned which entity, at which checkpoint.
 * aliases    : a normalised title -> node id, so cross-agent mentions collapse.
 * delegations: parent source -> child source.
 *
 * Idempotent and additive: runs only while `sources` is empty, and everything it
 * writes can be recomputed from the session files plus the existing checkpoint rows.
 */
function ensureProvenance(db) {
  const have = db.prepare('SELECT COUNT(*) AS n FROM sources').get();
  if ((have?.n ?? 0) > 0) return;
  const root = join(dshHome(), 'sessions');
  if (!existsSync(root)) return;
  const insertSource = db.prepare('INSERT OR IGNORE INTO sources (session_id, parent_session, agent_kind, cwd, label, created_at, last_seen) VALUES (?,?,?,?,?,?,?)');
  let found = 0;
  let projects = [];
  ...
  for (const project of projects) {
    ...
    let segments = [];
    try { segments = readdirSync(projectPath, { withFileTypes: true }); } catch { continue; }
    for (const segment of segments) {
      if (!segment.isDirectory()) continue;
      const file = pickGenerationFile(join(projectPath, segment.name));
      if (file === null) continue;
      let header = null;
      try { header = JSON.parse(readHeaderLine(file)); } catch { continue; }
      if (header === null || header.type !== 'session' || header.id === undefined) continue;
      const isSubagent = header.origin === 'subagent' || header.parentSession !== undefined || Number(header.delegationDepth ?? 0) > 0;
      let mtime = null;
      try { mtime = Math.round(statSync(file).mtimeMs); } catch { /* keep null */ }
      insertSource.run(String(header.id), header.parentSession === undefined ? null : String(header.parentSession),
        isSubagent ? 'subagent' : 'main', header.cwd === undefined ? null : String(header.cwd), null,
        header.createdAt === undefined ? null : Number(header.createdAt), mtime);
      found++;
    }
  }
  db.exec('INSERT OR IGNORE INTO mentions (source_id, node_id, checkpoint_seq, count, first_seen, last_seen) SELECT s.id, cn.node_id, cn.seq_start, 1, s.created_at, s.last_seen FROM checkpoint_nodes cn JOIN sources s ON s.session_id = cn.session_id');
  db.exec("INSERT OR IGNORE INTO aliases (alias, node_id, kind, created_at) SELECT lower(replace(replace(title,' ','-'),'_','-')), id, kind, first_seen FROM nodes");
  db.exec('INSERT OR IGNORE INTO delegations (parent_source, child_source) SELECT p.id, c.id FROM sources c JOIN sources p ON p.session_id = c.parent_session WHERE c.parent_session IS NOT NULL');
  console.warn('[dsh-session-handoff] provenance backfill: ' + found + ' source(s)');
}
...
/** A watermark is either a legacy bare lastSeq number or a resume record. */
function wmRecord(wm, sessionId) {
  const cur = wm[sessionId];
  if (cur && typeof cur === 'object') {
    return {
      lastSeq: Number(cur.lastSeq) || 0,
      byteOffset: Number(cur.byteOffset) || 0,
      size: Number(cur.size),
      mtimeMs: Number(cur.mtimeMs),
      prefixHash: Number(cur.prefixHash),
      hasCompaction: cur.hasCompaction === true,
    };
  }
  return { lastSeq: typeof cur === 'number' ? cur : 0, byteOffset: 0, size: NaN, mtimeMs: NaN, prefixHash: NaN, hasCompaction: undefined };
}

/** Persist the resume record for one session (byte offset + change token). */
function wmStore(wm, sessionId, seq, extra) {
  wm[sessionId] = { lastSeq: Number(seq) || 0, ...extra };
}
...
function linkCheckpointNodes(db, sessionId, seqStart, seqEnd, summary, ids, seq) {
  db.prepare('INSERT OR REPLACE INTO checkpoints (session_id, seq_start, seq_end, summary, shadowed_seqs, created_at) VALUES (?,?,?,?,?,?)').run(sessionId, seqStart, seqEnd, summary, '[]', Date.now());
...
function ingestSession(db, sessionId, events, wm, force, opts) {
  let lastSeq = wmRecord(wm, sessionId).lastSeq;
  let added = 0;
  // PTC/agent 后端不产 compaction/summary 事件：检测是否有标准事件，
  // 没有则用 assistant/message 按 turn 降级建图。
  // Incremental batches may hold no compaction event; trust the remembered
  // verdict so the turn-based fallback never fires for a compaction session.
  const hasCompaction = events.some((e) => e?.type === 'compaction/summary')
    || (opts != null && opts.hasCompactionHint === true);
  let lastTurn = -1;
...
export function registerGraphTools(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (cmd === 'build') {
        ...
        // The accessor, not `session.events` (which does not exist on a harness session). This line
        // used to be a ternary with the SAME expression in both branches, backed by that absent
        // property - so the documented fallback for non-zstd / plaintext sessions could never run,
        // and `build` silently ingested nothing whenever the session file held no events.
        const memEvents = sessionEvents(session);
  ...
  // Auto-build hook: no-LLM incremental ingest of a session file into the graph.
  // Reads the session FILE (compaction/summary events live in the shadowed
  // region), ingests only NEW checkpoints (watermark-guarded), writes watermarks.
  // Safe to call on every plugin apply — no new checkpoints = no-op.
  // Called by index.js after tool registration so every activated session is
  // indexed without a manual acp_graph build.
  return { autoIngestSession };
}

/** Incrementally index a session file into the graph. Returns { added, nodes, edges, checkpoints } or null on failure. */
export function autoIngestSession(sessionId, cwd) {
  try {
    const db = openDb();
    if (!db) return null;
    const fpath = sessionFilePath(sessionId, cwd);
    ...
    const wm = readWatermarks();
    const rec = wmRecord(wm, sessionId);
    const stamp = fileStamp(fpath);
    // Cheap gate: an unchanged log is a no-op. The watermark used to guard only
    // the DB write, so every compress still paid a full read + decode + parse
    // (measured 9.1 s / +993 MB on a 155 MB session) for nothing.
    if (rec.size === stamp.size && rec.mtimeMs === stamp.mtimeMs && rec.prefixHash === stamp.prefixHash) {
      const s0 = graphStats(db);
      return { added: 0, unchanged: true, nodes: s0.nodes, edges: s0.edges, checkpoints: s0.checkpoints, bytesParsed: 0 };
    }
    // Session logs are append-only: resume at the last consumed frame boundary.
    // A rewritten file (prefix changed) or a shrunken one restarts from zero.
    const from = (rec.prefixHash === stamp.prefixHash && rec.byteOffset > 0 && rec.byteOffset <= stamp.size) ? rec.byteOffset : 0;
    const inc = readSessionEventsIncremental(fpath, from);
    const added = ingestSession(db, sessionId, inc.events, wm, false, { hasCompactionHint: rec.hasCompaction });
    const hasCompaction = rec.hasCompaction === true || inc.events.some((e) => e?.type === 'compaction/summary');
    wmStore(wm, sessionId, wm[sessionId], {
      byteOffset: inc.consumedBytes, size: stamp.size, mtimeMs: stamp.mtimeMs, prefixHash: stamp.prefixHash, hasCompaction,
    });
    writeWatermarks(wm);
    const s = graphStats(db);
    return { added, nodes: s.nodes, edges: s.edges, checkpoints: s.checkpoints, bytesParsed: inc.consumedBytes - from };
  } catch (e) {
    console.warn('[dsh-session-handoff] autoIngestSession failed:', String(e?.message ?? e));
    return null;
  }
}
