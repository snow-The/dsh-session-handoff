...
/**
 * Point agent-default-model at one route serving MODEL_ID and persist it to
 * settings.yaml. Shared by the model_switch tool and the web client route.
 * Returns the effective selection (provider name incl. the vision variant).
 */
async function switchProvider(ctx, provider, vision) {
  ...
  const useVision = vision === true && route.visionVariant != null;
  const effective = useVision ? route.visionVariant : route.provider;
  ...
  return { route, provider: effective, useVision };
}
...
function registerModelRoutesTools(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'string' }, render: (_a, v) => [{ type: 'text', text: v }] },
    async execute(args) {
      const { route, provider, useVision } = await switchProvider(ctx, args?.provider, args?.vision === true);
