  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        return `compressed${args.topic ? ` (${args.topic})` : ''} ${result.shadowedRange.start}-${result.shadowedRange.end}: ${result.shadowedSeqs?.length ?? 0} nodes` + (stored ? ` | blocks saved: ${stored}` : '');
      } catch (error) {
