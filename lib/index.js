...
function detectEnhancers(ctx) {
  ...
  try {
    ...
    if (viking.length) found.push(`OpenViking memory: ${viking.map((t) => t.name).join(', ')} (agent can archive this handoff via viking_remember)`);
  } catch { /* tools.list may not exist */ }
  // archify: check the archify skill dir / CLI presence.
  try {
    const skillRoot = resolve(process.cwd(), '.dsh', 'skills');
    const archifySkill = existsSync(join(skillRoot, 'archify'));
    if (archifySkill) found.push('archify: skill available (agent can generate a progress diagram)');
  } catch { /* ignore */ }
  return found;
}
...
function registerHandoffTools(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      const doc = renderHandoffDoc(ctx, agent, events, { enhancers });
      ...
      if (enhancers.some((e) => e.startsWith('OpenViking'))) {
        lines.push('Tip: run viking_remember with this handoff content to archive it for cross-session recall.');
      }
      return lines.join('\n');
    },
  }));
