...
function registerAcpTools(ctx, config, pending, windows, completionBudgets) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      // NOTE: never write `required: false` — the host's schema compiler rejects the key
      // outright ("parameters.X.required must be true when present") and the whole plugin
      // tree then fails to load. Omit the key for optional parameters.
...
export function apply(ctx, config) {
  ...
  async function hostTriggeredFold(agent) {
    try {
      ...
      const nodes = session.surface?.nodes ?? [];
      if (nodes.length <= (config.preserveRecent ?? 2) + 1) return;   // nothing safe to fold
      const before = used;
      await acpTools.runCompaction(agent, { end: 'auto', start: nodes[0], deep: false, summary: null, topic: 'host-trigger' });
