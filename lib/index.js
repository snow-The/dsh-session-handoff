...
const sessionFileCache = new Map(); // sessionId -> { mtime, bySeq: Map<seq, event> }
function getSessionFileEvents(session) {
  try {
    const id = session?.id;
    if (!id) return new Map();
    const cwd = session?.header?.cwd ?? process.cwd();
    const path = sessionFilePath(id, cwd);
    if (!path) return new Map();
    let mtime = 0;
    try { mtime = statSync(path).mtimeMs; } catch { return new Map(); }
    const hit = sessionFileCache.get(id);
    if (hit && hit.mtime === mtime) return hit.bySeq;
    const events = readSessionEventsFromFile(path);
    // build seq -> event index (array position != event.seq)
    const bySeq = new Map();
    for (const e of events) if (e && e.seq != null) bySeq.set(Number(e.seq), e);
    sessionFileCache.set(id, { mtime, bySeq });
    return bySeq;
...
function surfaceNodes(ctx, session) {
  ...
  const file = getSessionFileEvents(session);
