...
function registerAcpTools(ctx, config, pending, windows) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      return `compaction backend: ${plane}\n` + renderStatus(ctx, agent, config, w);
    },
  }));
...
function estimateGrowth(ctx, agent, turns = 5) {
  try {
    ...
    // Only count nodes still on the current surface: ACP compression hides
    // older seqs between the turn markers, and those hidden nodes no longer
    // occupy context. Counting them would inflate growth by orders of
    // magnitude right after a compression (e.g. 600k/turn) and falsely show
    // "~0 turns until hard limit", triggering premature compression.
