...
function renderStatus(ctx, agent, config, windowTokens, completionBudget, opts = {}) {
  ...
  if (opts.quiet) {
    const pct = Number.isFinite(minTokens) && minTokens > 0 ? Math.round((used / minTokens) * 100) : null;
    return `context: ${used}${windowTokens ? ` / ${windowTokens}` : ''} tokens (level ${level}${pct != null ? `, ${pct}% of soft limit` : ''})`;
  }
  ...
  if (systemPrompt != null) {
    ...
    systemPrompt.context({
      ...
      text: (assembly) => {
        ...
        if (!showFull) {
          const pct = Number.isFinite(soft) && soft > 0 ? Math.round((used / soft) * 100) : null;
          return `ACP: ${used}${w ? `/${w}` : ''} tokens${pct != null ? ` (${pct}% of soft ${soft})` : ''} — below soft limit, no compaction needed. Keep working.`;
        }
