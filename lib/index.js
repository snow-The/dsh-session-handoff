...
export function renderStatus(ctx, agent, config, windowTokens, completionBudget, opts = {}) {
  ...
  // Quiet mode: a one-liner used for the per-turn injection below the soft
  // limit. The surface dump is only rendered when the model actually needs it
  // (it exists solely to pick seq ids for acp_compress).
  if (opts.quiet) {
    // Same rule as the banner: report fullness against the WINDOW, and mention the soft
    // limit as a threshold — never as a bare percentage that reads like "context full".
    const pctWindow = Number.isFinite(windowTokens) && windowTokens > 0 ? Math.round((used / windowTokens) * 100) : null;
    const pctSoft = Number.isFinite(minTokens) && minTokens > 0 ? Math.round((used / minTokens) * 100) : null;
    const softLeft = Number.isFinite(minTokens) && minTokens > 0 ? Math.max(0, minTokens - used) : null;
    const hardLeft = Number.isFinite(maxTokens) && maxTokens > 0 ? Math.max(0, maxTokens - used) : null;
    return `context: ${used}${windowTokens ? ` / ${windowTokens}` : ''} tokens (level ${level}`
      + `${pctWindow != null ? `, ${pctWindow}% of window` : ''}${softLeft != null ? `, ${Math.round(softLeft / 1000)}k below soft ${minTokens}` : ''}${hardLeft != null ? `, ${Math.round(hardLeft / 1000)}k below hard ${maxTokens}` : ''})`;
  }
...
function registerAcpTools(ctx, config, pending, windows, completionBudgets) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (Number.isNaN(end) || String(args.end ?? '').trim().toLowerCase() === 'auto') {
      ...
      let foldedFrom = null;
      if (args.deep !== false && Number.isFinite(start)) {
...
/**
 * Last window we could resolve, for ANY session. The per-turn banner renders synchronously
 * and used to read only the per-session map — which agent/pre-step fills — so at session
 * start (and after a restart) the banner saw no window at all: no percentage, and worse,
 * soft = Infinity meant the ACP threshold could not fire. The window is a property of the
 * MODEL, so remembering the last one is both correct and enough.
 */
let lastKnownWindow;
...
async function rememberWindow(ctx, agent) {
  ...
  } catch { /* a window probe must never break a turn */ }
  return null;
}
...
export function apply(ctx, config) {
  ...
  if (systemPrompt != null) {
    ...
    systemPrompt.context({
      ...
      text: (assembly) => {
        ...
        if (!showFull) {
          // Lead with the WINDOW percentage. A bare "100%" measured against the soft limit
          // reads as "the context is full" and is simply false — the window is w (1,000,000
          // here), so a context at the 730k soft limit is 73% full, not 100%.
          const pctWindow = Number.isFinite(w) && w > 0 ? Math.round((used / w) * 100) : null;
          const pctSoft = Number.isFinite(soft) && soft > 0 ? Math.round((used / soft) * 100) : null;
          // "soft limit 730000 = 75%" was read as "the soft limit IS 75%" — the reported
          // ratio and the threshold number sat side by side with no separator of meaning.
          // Headroom cannot be misread: state how much is left before the threshold fires.
          const parts = [];
          if (pctWindow != null) parts.push(`${pctWindow}% of the window`);
          if (Number.isFinite(soft) && soft > 0) {
            const left = Math.max(0, soft - used);
            parts.push(left > 0 ? `soft limit ${soft} — ~${Math.round(left / 1000)}k left` : `soft limit ${soft} — reached`);
            ...
            if (Number.isFinite(hard) && hard > 0) {
              const hardLeft = Math.max(0, hard - used);
              parts.push(hardLeft > 0 ? `hard ${hard} — ~${Math.round(hardLeft / 1000)}k left` : `hard ${hard} — reached`);
            }
          }
          return `ACP: ${used}${w ? `/${w}` : ''} tokens${parts.length ? ` (${parts.join('; ')})` : ''} — below soft limit, no compaction needed. Keep working.`;
        }
  ...
  registerAcpTools(ctx, config, pending, windows, completionBudgets);
