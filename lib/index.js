...
export { sessionFileIndex as __sessionFileIndex };
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        // index it immediately (no-LLM incremental, watermark-guarded).
        try {
          const ag = autoIngestSession(agent?.session?.id, agent?.session?.header?.cwd);
          if (ag && ag.added > 0) console.warn('[dsh-session-handoff] graph auto-ingest: +' + ag.added + ' checkpoint(s)');
        } catch (e) { console.warn('[dsh-session-handoff] graph auto-ingest skipped:', String(e?.message ?? e)); }
