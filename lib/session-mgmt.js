...
/**
 * The persistence list returns SNAPSHOTS — `{ header: { id, cwd }, revision, sizeBytes }` —
 * not bare headers. Reading `h.id` off a snapshot yields undefined for every row, which is
 * how session_list came to print "undefined" lines and session_trash kept reporting
 * "(no artifact found)": both consumers looked up a field that does not exist at that depth.
 * Entries without an identity are dropped rather than rendered.
 */
export function sessionEntries(headers) {
  if (headers == null) return [];
  const out = [];
  for (const h of headers) {
    const header = h?.header ?? h;
    const id = header?.id;
    if (typeof id !== 'string' || id === '') continue;
    out.push({ entry: h, header, id, cwd: header?.cwd ?? '' });
  }
  return out;
}

/** Match an id with or without the optional `session-` prefix, either direction. */
function sameId(a, b) {
  const strip = (s) => String(s).replace(/^session-/, '');
...
async function locateSession(ctx, sessionId) {
  try {
    ...
    const meta = sessionEntries(headers).find((e) => sameId(e.id, sessionId));
    if (meta == null) return undefined;
    const location = ctx.sessionPersistence?.locate?.(meta.entry);
...
export function registerSessionMgmtTools(ctx) {
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      try {
        const headers = await ctx.sessionPersistence?.list?.() ?? [];
        for (const e of sessionEntries(headers)) {
          const live = ctx.agents?.get?.(e.id) !== undefined;
          lines.push(`${e.id}${live ? ' (running)' : ''}  ${e.cwd}`);
        }
