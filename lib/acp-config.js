...
export function registerAcpConfigTools(ctx) {
  ctx.tools.register(defineTool({
    ...
    async execute() {
      ...
      return [
        ...
        `  nudge: ${cfg.nudge}`,
        '',
        'Takes effect for new sessions; the current session keeps its own.',
      ].join('\n');
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      return [
        'ACP thresholds updated in settings.yaml:',
        `  minContextLimit: ${next.minContextLimit}`,
        `  maxContextLimit: ${next.maxContextLimit}`,
        `  preserveRecent: ${next.preserveRecent}`,
        `  minTokens: ${next.minTokens}`,
...
export { readSection, validateLimit };
