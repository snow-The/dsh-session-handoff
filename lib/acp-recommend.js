 ...
 *   2. compaction cost — MEASURED: one summarize call AND one full-prefix cache miss.
 *      A surface replacement bumps replaceGeneration, so the host re-projects the system
 *      prompt and opens a new request series; the next call re-sends the WHOLE prompt at the
 *      miss rate (≈10× the cached rate). That is ≈9 turns of carrying cost, which dominates
 *      the summarize call. Consequence: fewer, later, deeper compactions beat many shallow
 *      ones — event count ∝ 1/(soft − foldTarget), so a HIGHER soft means FEWER misses.
 *      (Old note: one summarize call that reads the entire compacted
...
export function recommendThresholds({
  ...
  growthPerTurn,
  hardRatio = 0.9,
  // Reaction runway, not a safety cushion: with the miss cost above, the cheap move is to
  // compact as FEW times as possible, so soft tracks hard. 4 turns is enough to notice the
  // banner and fold; the previous 6 both fired earlier and produced more misses.
  bufferRounds = 4,
  minSoftRatio = 0.35,
  maxTokens,
} = {}) {
  ...
  return {
    ...
    reasoning: [
      ...
      'each compaction also costs one full-prefix cache miss (≈9 turns of carrying cost at miss=10×hit), so the aim is FEWEST events: soft tracks hard and each fold goes deep',
