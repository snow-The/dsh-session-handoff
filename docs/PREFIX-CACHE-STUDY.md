...
## 七、后续实测（同日晚）：两条被数据否决的提案

### 7.1 提案 4（轮末收缩工具结果）在 DSH 里**有害** ✗

Reasonix 的做法（Pillar 3：轮末把 >3000 token 的工具结果压到 3000）在**我们的宿主**上是反效果：
任何 surface 编辑都会 bump `replaceGeneration` → 宿主重投影 system prompt + 开新请求系列
（`dsh-agent-loop/lib/index.js:1019`）→ **一次全量前缀 miss** ✗。
每轮收缩 = **每轮一次全量 miss** ✗，比现状（长会话 135–168 次压缩）更糟。

**缓存安全的替代（未实现）**：在**追加时**就限制大工具结果 ✗（只追加、不重写 ✓），
需要宿主在 tool-result 处提供钩子 ✓。

### 7.2 提案 5（storm 抑制）实测**不成立** ✗

用本会话自己的日志（614 次工具调用）分类"相同 `(tool,args)` 重复"的动机：
...
即：Reasonix 式抑制会**砍掉约 7 倍于收益的合法重试** ✗✗ —— 本会话全靠重试从 Windows 路径、
超时、内存分配失败里恢复 ✓，那正是抑制规则要拦下的行为 ✗。

**决定**：不实现抑制 ✓；真浪费只占 1.3% ✓，连提示都不值得 ✗。
（附注：用"结果里是否出现 error 字样"来判定失败**不可靠** ✗ —— 本会话它把 545/614 判成失败 ✓，
因为脚本自己会打印 error 字样 ✓。要判定成败必须读结构化字段 ✓。）

### 7.3 提案 2（深度折叠）已实现 ✓

见 `acp_compress` 的 `deep` 默认行为 + `compressTargetRatio`（默认 0.35 窗口）✓。
验收：`acp_status` 的 "compactions this session" 增速与 `prefix cache` 命中率 ✓
（历史基线：长会话 135/150/168 次压缩、命中率 69.8% ✗）。
